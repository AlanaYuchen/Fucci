...
track$lineageId = track$trackId # erase original lineage ID, assign in following steps
...
potential_daughter_pair_id = intersect(ann$track[grep('[M,G1]', ann$app_stage)], ann$track[which(ann$mitosis_identity==F)]) # daughter track must appear as M during mitosis
...
potential_daughter_trackId = sub_ann$track[grep('[M,G1]', sub_ann$app_stage)] # potential daughter tracks must appear at M phase during mitosis
